...
class AppleAuthCubit extends Cubit<AppleAuthState> {
  AppleAuthCubit(this.registerRepository) : super(const AppleAuthState());

  final RegisterRepository registerRepository;
  Future<void> registerOrLoginWithApple() async {
    emit(state.copyWith(formStatus: FormStatus.submitting));
    try {
      final response = await registerRepository.registerOrLoginWithApple();
      await response.fold(
        (failure) {
          emit(
            state.copyWith(
